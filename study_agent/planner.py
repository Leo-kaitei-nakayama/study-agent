"""シラバスを読んで「この科目で何を使うか」を組み立てる (tool strategy)。

同期のときに 1 回だけ走り、結果は courses.tools に JSON で入る。
router.py は下書きを作るとき、この結果を LLM への指示に混ぜる
(「この科目は C++17 と Valgrind を使う」と分かっていれば的外れな下書きが減る)。
...
webapp/coursemap.py はここを読んで再公開しているだけなので、
**語彙の本体はこのファイルにしかない**(二重管理を避けるため)。
"""
...
# ------------------------------------------------------------ tool strategy
def build_tool_strategy(syllabus: str, assignments: list[dict] | None = None) -> dict:
    """科目 1 つぶんの「使う道具」をまとめて返す。

    返す形(courses.tools にそのまま入る):

        {"tools":      [{"name": "C++17", "category": "language"}, ...],
         "by_category": {"language": ["C++17"], "debug": ["Valgrind", "GDB"]},
         "summary":    "C++17 · GCC · Valgrind",     # LLM への 1 行説明
         "source":     "syllabus+assignments"}

    assignments を渡すと課題の説明文も一緒に読む。シラバスが薄い科目
    (本体が外部サイトにある)でも、課題文から道具を拾えることがあるため。
    """
    texts = [syllabus or ""]
    ...
    return {
        ...
        "summary": " \u00b7 ".join(t["name"] for t in tools),
        "source": "syllabus+assignments" if used_assignments else "syllabus",
    }


def strategy_from_json(raw: str | None) -> dict:
    """courses.tools を読み戻す。

    古い行には道具の配列がそのまま入っている(build_tool_strategy を入れる前の
    形)。どちらで入っていても同じ形で返す — 移行のために作り直す必要はない。
    """
    if not raw:
        return {"tools": [], "by_category": {}, "summary": "", "source": ""}
    try:
        data = json.loads(raw)
    except (ValueError, TypeError):
        return {"tools": [], "by_category": {}, "summary": "", "source": ""}

    if isinstance(data, list):                       # 昔の形(配列だけ)
        tools = [d for d in data if isinstance(d, dict) and d.get("name")]
        by_category: dict[str, list[str]] = {}
        for t in tools:
            by_category.setdefault(t.get("category", "other"), []).append(t["name"])
        return {"tools": tools, "by_category": by_category,
                "summary": " \u00b7 ".join(t["name"] for t in tools),
                "source": "legacy"}
