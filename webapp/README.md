...
上部に **科目のタブ**(`?course=<id>`)がある。オリエンテーション科目だけで
100 件を超えることがあり、全部を1つの流れで出すと目当ての授業が埋もれる
ため。既定は「すべての科目」。学期タブを押しても科目の絞り込みは外れない。
存在しない course が来たら「すべて」に落とす(壊さない)。
...
答えは小窓にその場で出るだけでなく、**ノート `kind='screenshot'` として
保存される**。科目名を入れて撮ればその科目のページに、入れなければ
「未分類」に入る。一覧では 📷「画面への質問」として出る。

## 自分で撮ったスクショで質問する (`/ask`)

拡張機能の自動キャプチャは Canvas では止めてある(学生が Canvas を操作すると
決めたため)。その代わりに **撮るのは本人** という入口を用意した。

**拡張機能から macOS のスクリーンショット機能は呼べない。** 拡張機能は
ブラウザの中から出られないので、OS のショートカットを押したりアプリを
起動したりはできない(native messaging で別の実行ファイルを入れれば別だが、
そこまでする話ではない)。代わりにブラウザの標準機能だけで受け取る:

| 渡し方 | 手順 |
|---|---|
| 貼り付け | `⌃⌘⇧4` でクリップボードに撮る → 画面のどこかで `⌘V` |
| ドラッグ | `⌘⇧4` で保存した .png を枠に落とす |
| ファイル選択 | いつもの入力欄(**JS を切っていてもこれは動く**) |

貼り付けとドラッグは JS が data URL にして隠しフィールドに入れる。
サーバー側は `_read_uploaded_image()` がどちらの形でも読む。

答え方は拡張機能と同じ 3 つ(説明 / 下書き / 自由指示)で、処理は
`_answer_about_image()` に 1 本化してある。**どちらの入口でも提出はしない。**
結果は画面に出るほか、ノート `kind='screenshot'` として残る。科目を選べば
その科目のページに入る。
