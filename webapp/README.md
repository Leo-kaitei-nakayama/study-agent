...
「履修予定表を取り込む」に Study List の `.html` を入れると、いま履修中の
科目が成績 **N/A** で一覧に入る。N/A の科目は:

- GPA にも取得単位にも数えない(「履修中の単位」として別に集計)
- 成績が出たら、その行のプルダウンから入力できる
- 成績を入れると source が `manual` に変わるので、予定表を貼り直しても消えない

### 画面の畳み方

学期が増えると縦に長くなるので、既定では**最新の 1 学期だけ**を開き、
残りは「すべて表示」(`?all=1`)で開く。成績表の取り込みも、一度取り込んだ
後は右端の小さなボタンに畳まれる。
